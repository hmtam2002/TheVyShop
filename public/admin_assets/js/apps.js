...
function createSortFiler() {
    if ($("#jFilerSortable").length) {
        sortable = new Sortable.create(
            document.getElementById("jFilerSortable"),
            {
                animation: 600,
                swap: true,
                disabled: true,
...
function login() {
    ...
    if ($(".show-password").hasClass("active")) {
        ...
        $(".show-password")
            .find("span")
            .toggleClass("fas fa-eye fas fa-eye-slash");
    }
...
$(document).ready(function () {
    ...
    if ($(".regular_price").length && $(".sale_price").length) {
        $(".regular_price, .sale_price").keyup(function () {
            ...
            var sale_price = $(".sale_price").length
                ? $(".sale_price").val()
                : 0;
            var discount = 0;
    ...
    if ($("#delete-all").length) {
        $("body").on("click", "#delete-all", function () {
            ...
            confirmDialog(
                "delete-all",
                "Bạn có chắc muốn xóa những mục này ?",
                url
            );
        });
    }
    ...
    if ($(".form-control-ckeditor").length) {
        CKEDITOR.editorConfig = function (config) {
            ...
            /* All Plugins */
            config.extraPlugins =
                "image2,codemirror,texttransform,copyformatting,html5video,html5audio,flash,youtube,wordcount,tableresize,widget,lineutils,clipboard,dialog,dialogui,widgetselection,lineheight,video,videodetector";
            ...
            if (
                ...
                var folderCkfinder =
                    PHP_VERSION >= 8 ? "ckfinder8" : "ckfinder7";
                config.filebrowserBrowseUrl =
                    folderCkfinder + "/ckfinder.html?token=" + TOKEN;
                ...
                config.filebrowserBrowseUrl =
                    "elfinder/index.php?token=" + TOKEN;
            }
