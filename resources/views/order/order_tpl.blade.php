...
$sluglang = 'slugvi';
$lang = 'vi';
$func = new \App\Models\FunctionModel();
class Flash {
	function has($key) {
		return true;
	}
...
<form class="form-cart validation-cart" novalidate method="post" action="" enctype="multipart/form-data">
	<div class="wrap-cart">
		<?php // echo $flash->getMessages("frontend") ?>
		<div class="row">
			<?php if (!empty($cart)) { ?>
				<div class="top-cart col-12 col-lg-7">
					...
					<div class="list-procart">
						...
						for ($i = 0; $i < $max; $i++) {
							$pid = $cart['productid'];
							$quantity = $cart['qty'];
							$color = ($cart['color']) ? $cart['color'] : 0;
							$size = ($cart['size']) ? $cart['size'] : 0;
							$code = ($cart['code']) ? $cart['code'] : '';
							// $proinfo = $cart->getProductInfo($pid);
							$proinfo = Product::where(['id' => $pid])->first();
							if ($color > 0) {
								// $colorInfo = $cart->getColorInfo($color);
								$colorInfo = Color::where(['id' => $color])->first();
								...
								<div class="form-row">
									<div class="pic-procart col-3 col-md-2">
										<a class="text-decoration-none" href="<?= $proinfo[$sluglang] ?>" target="_blank" title="<?= $proinfo['name' . $lang] ?>">
											<img onerror="this.src='{{ asset("thumbs/85x85x2/assets/images/noimage.png") }}';" src="{{ asset('thumbs/85x85x2/upload/product/' . $pictureItems) }}" alt="<?= $proinfo['name' . $lang] ?>" title="<?= $proinfo['name' . $lang] ?>" />
										</a>
									...
									<div class="info-procart col-6 col-md-5">
										...
										<div class="properties-procart">
											<?php if ($color) {
												...
												$color_detail = Color::where(['type' => $proinfo['type'], 'id' => $color])->first(); ?>
					...
					<div class="money-procart">
						<?php if ($config['order']['ship']) { ?>
							<div class="total-procart d-flex align-items-center justify-content-between">
								<p><?= tamtinh ?>:</p>
								<p class="total-price load-price-temp"><?= $func->formatMoney($total) ?></p>
							</div>
						<?php } ?>
						<?php if ($config['order']['ship']) { ?>
							<div class="total-procart d-flex align-items-center justify-content-between">
						...
						<div class="total-procart d-flex align-items-center justify-content-between">
							<p><?= tongtien ?>:</p>
							<p class="total-price load-price-total"><?= $func->formatMoney($total) ?></p>
						</div>
					</div>
				</div>
